...
export const resendVerificationEmail = async (req, res) => {
  try {
    ...
    // FIX 1: Check the correct field name
    if (user.isEmailVerified) {  // Changed from user.verified
      return res.status(400).json({
        success: false,
    ...
    // FIX 2: Check if the field exists or use a default
    const lastSentAt = user.verificationSentAt || user.emailVerificationSentAt || 0;
    ...
    user.verificationSentAt = new Date(); // Make sure this field exists in schema
    await user.save();

    // Send verification email
    await sendVerificationEmail(user, verificationToken);

    // Log to audit log - FIX: Use valid enum value
    try {
      await AuditLog.create({
        userId: user._id,
        action: "verification_resent", // This should be in your enum
        details: "Resent verification email",
        ip: getClientIP(req),
        userAgent: req.headers["user-agent"],
        metadata: { 
          attempts: attempts,
      ...
      console.warn("Audit log error (non-critical):", auditError.message);
    ...
    res.json({
      ...
      data: {
        ...
        email: user.email,
        resendAt: new Date(),
        nextResendAvailable: new Date(Date.now() + 60000) // 1 minute from now
      }
    });
